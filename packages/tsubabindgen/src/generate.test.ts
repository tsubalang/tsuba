...
describe("@tsuba/tsubabindgen", () => {
  ...
  function runTempFixture(options: {
    readonly fixture: "simple" | "traits" | "advanced" | "edge" | "reexports" | "parse-failure";
  ...
  it("resolves explicit public re-exports and reports unsupported glob re-exports", () => {
    const out = runTempFixture({ fixture: "reexports", packageName: "@tsuba/reexports", bundleCrate: false });
    const rootDts = read(join(out, "index.d.ts"));

    expect(rootDts).to.include("export const ROOT_ANSWER: i32;");
    expect(rootDts).to.include("export declare class Thing {");
    expect(rootDts).to.include("export declare class RootHelper {");
    expect(rootDts).to.include("export function make_thing(value: i32): Thing;");
  ...
  it("records Rust parser failures as skip issues without failing generation", () => {
    const out = runTempFixture({ fixture: "parse-failure", packageName: "@tsuba/parse-failure", bundleCrate: false });
    const rootDts = read(join(out, "index.d.ts"));
    const badDts = read(join(out, "bad.d.ts"));

    expect(rootDts).to.include("export const ROOT_OK: i32;");
    expect(badDts).to.include("// Generated by tsubabindgen (v0)");
